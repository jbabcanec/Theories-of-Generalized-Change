...
\begin{enumerate}
    ...
    \begin{enumerate}
        ...
        \]
        \item \textbf{Range:} The numerator \( \ln(x - y) \) can take any real value since \( x - y > 0 \), and the denominator \( \sqrt{x^2 + y^2 - 4} \) is strictly positive and increases as \( (x, y) \) moves farther from the circle \( x^2 + y^2 = 4 \). The output \( g(x, y) \) cannot be zero because the numerator \( \ln(x - y) \) cannot simultaneously be zero (i.e., \( x - y = 1 \)) while satisfying the conditions imposed by the square root denominator. Therefore, the range is:
        \[
        \range g = \mathbb{R}.
        \]
    \end{enumerate}
...
\end{tikzpicture}
\caption{\textit{A 3D surface featuring with its level curves shown below featuring the Bernoulli lemniscate (the infinity shaped level curve)}}
\end{figure}
...
\]
where \( \mathbf{x} \) represents a point in \( \mathbb{R}^3 \). This column vector-based notation is particularly valuable in advanced topics like gradients, divergence, and vector-valued functions, as it simplifies both expression and computation.
...
\noindent Plotting a level surface, will be very similar to our level curves. lets take a look at a hypersphere \(f(x, y, z) = x^2 + y^2 + z^2\). The level surfaces are determined by solving:
\[
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}
        ...
        \end{axis}
    \end{tikzpicture}
    \caption{\textit{Level surfaces of \( f(x, y, z) = x^2 + y^2 + z^2 = k\) for \( k = 1, 4, 9 \), represented as concentric circles}}
...
\begin{tikzpicture}
  ...
  \end{axis}
\end{tikzpicture}
    \caption{\textit{3D plot with a \(\delta\)-circle, center point \((a, b)\), a transparent surface, \(f(a,b)\), the circle plotted on the surface, and the corresponding points \(L\), \(L + \epsilon\), and \(L - \epsilon\) on the \(z\)-axis connected to their corresponding surface points.}}
\end{figure}
...
\noindent \textbf{Proof:} Let \( \epsilon > 0 \) be given. We aim to find \( \delta > 0 \) such that if \( 0 < \sqrt{x^2 + y^2} < \delta \), then \( |x y| < \epsilon \). Note that:
\[
|x y| = |x||y| \leq \left( \sqrt{x^2} \right) \left( \sqrt{y^2} \right) = |x||y|.
\]
\noindent But since \( |x| < \sqrt{x^2 + y^2} \) and \( |y| < \sqrt{x^2 + y^2} \) for $x,y \neq 0$ we have:
\[
|x y| < \left( \sqrt{x^2 + y^2} \right) \left( \sqrt{x^2 + y^2} \right) = x^2 + y^2.
\]

\noindent Furthermore, since \( x^2 + y^2 = \left( \sqrt{x^2 + y^2} \right)^2 \), it follows that:
\[
|x y| < \left( \sqrt{x^2 + y^2} \right)^2.
\]

\noindent Now, choose \( \delta = \sqrt{\epsilon} \). Then \( \delta > 0 \) because \( \epsilon > 0 \). Suppose \( 0 < \sqrt{x^2 + y^2} < \delta \). Then:
\[
|x y| < \left( \sqrt{x^2 + y^2} \right)^2 < \delta^2 = (\sqrt{\epsilon})^2 = \epsilon.
\]
...
\begin{itemize} \index{Intermediate Value Theorem}
    ...
    \item \textbf{Mean Value Theorem:} \index{Mean Value Theorem} For multivariable functions, this theorem generalizes to state that if \( f \) is continuous and differentiable on a region \( R \), then there exists a point \( \mathbf{x}_c \in R \) such that the gradient (multi-dimensional derivative) \( \nabla f(\mathbf{x}_c) \) aligns with the direction of the greatest change in \( f \) over \( R \). This result is crucial in applications such as error estimation and optimization.
...
\begin{tikzpicture}
  ...
  \end{axis}
\end{tikzpicture}
 \caption{\textit{Derivative of a surface $z = x^2 + y^2$ at a point $(1,1,2)$. The tangent line (red) is notated with proejctions of each component, green indicating $yz$-plane projection of the tangent line, and blue indicating the $xz$-plane projection.}}
 \label{fig:derivative_high_ordere}
\end{figure}
...
\begin{enumerate}
    ...
    \begin{itemize}
        ...
        Simplify:
        \[
        \frac{\partial h}{\partial y} = \frac{2xy - 2y^2 + x^2 + y^2}{(x - y)^2}.
        \]
    \end{itemize}
\end{enumerate}
\end{solution}
...
\noindent Surprisingly—or perhaps reassuringly—the answer is that the order generally does *not* matter, provided certain conditions are met. 
...
\noindent Compute the mixed partial derivatives at \( (0, 0) \):
\begin{itemize}
    \item \( \frac{\partial f}{\partial x} = \begin{cases} 
    \frac{y(x^2 - y^2)}{(x^2 + y^2)^2}, & \text{if } (x, y) \neq (0, 0), \\
    0, & \text{if } (x, y) = (0, 0).
    \end{cases} \)
    \item Taking \( \frac{\partial}{\partial y} \) and \( \frac{\partial}{\partial x} \) in different orders produces inconsistent results, as the second-order partial derivatives involve discontinuities in their behavior near \( (0, 0) \).
\end{itemize}

\noindent This failure arises because \( f(x, y) \) is not continuously differentiable at \( (0, 0) \), violating the conditions of Clairaut's Theorem. \\
...
\begin{enumerate}
    ...
    \begin{itemize}
        ...
        \item Compute \( \frac{\partial^2 f}{\partial x \partial y} \):
        \[
        \frac{\partial^2 f}{\partial x \partial y} = \frac{\partial}{\partial y} (ye^{xy}) = e^{xy} + y^2e^{xy}.
        \]
        \item Compute \( \frac{\partial f}{\partial y} \):
        \[
        ...
        \item Compute \( \frac{\partial^2 f}{\partial y \partial x} \):
        \[
        \frac{\partial^2 f}{\partial y \partial x} = \frac{\partial}{\partial x} (xe^{xy}) = e^{xy} + y^2e^{xy}.
        \]
        Since \( \frac{\partial^2 f}{\partial x \partial y} = \frac{\partial^2 f}{\partial y \partial x} = e^{xy} + y^2e^{xy} \), Clairaut's Theorem is verified.
    \end{itemize}
\end{enumerate}
\end{solution}
\end{example}
...
\begin{enumerate}
    ...
    \begin{itemize}
        ...
        \]
        \item Substitute \( x = e^u \) and \( y = \cos(v) \):
        \[
        \frac{\partial z}{\partial u} = \frac{1}{e^u\cos(v)} \cdot e^u = \frac{1}{\cos(v)}.
        \]
...
\[
\frac{\partial f}{\partial x} = g_u \frac{\partial h}{\partial x} = g_u (-y \sin(xy))
\]
...
&= \left( g_{uu} \frac{\partial h}{\partial y} + g_{uv} \frac{\partial k}{\partial y} \right) (-y \sin(xy)) + g_u \left( -\sin(xy) - x y \cos(xy) \right) \\
...
\begin{tikzpicture}
  ...
  \end{axis}
\end{tikzpicture}
\caption{\textit{A tangent line to the sphere \(x^2 + y^2 + z^2 = 9\) at the point \((2, 1, \sqrt{5})\). The tangent line is shown in red. The point of tangency is marked in black.}}
...
\begin{enumerate}
    ...
    The gradient vector \( \nabla f(x, y) = \langle f_x,f_y,f_z \rangle \) points in the direction of maximum increase of the function \( f(x, y) \). Its magnitude \( |\nabla f| \) reflects the steepness of the ascent at a point.
...
\begin{enumerate}
    ...
    \[
    \nabla f(0.5, 0.5) = \begin{bmatrix} -1.0 \\ -1.6 \end{bmatrix}.
    \]
    ...
    \[
    |-\nabla f(0.5, 0.5)| = \sqrt{(1.0)^2 + (1.6)^2} = \sqrt{1 + 2.56} = \sqrt{3.56} \approx 1.886.
    \]
    The unit vector pointing toward the center is:
    \[
    \hat{\mathbf{r}}_1 = \frac{-\nabla f(0.5, 0.5)}{|-\nabla f(0.5, 0.5)|} = \frac{1}{1.886} \begin{bmatrix} 1.0 \\ 1.6 \end{bmatrix} \approx \begin{bmatrix} 0.53 \\ 0.85 \end{bmatrix}.
    \]
    (Note: For simplicity in plotting, we've approximated \( \hat{\mathbf{r}}_1 \) to \( (-0.6, -0.8) \), maintaining the direction.)
    ...
    -First Step (\( \mathbf{P}_1 \) to \( \mathbf{P}_2 \)):
      \[
      \mathbf{P}_2 = \mathbf{P}_1 + h \hat{\mathbf{r}}_1 = (0.5, 0.5) + 0.3 \cdot (-0.6, -0.8) = (0.5 - 0.18, 0.5 - 0.24) = (0.32, 0.26)
      \]
      \[
      z_2 = 4 - (0.32)^2 - (0.26)^2 = 4 - 0.1024 - 0.0676 = 3.83
      \]
      Thus, \( \mathbf{P}_2 = (0.32, 0.26, 3.83) \).

    -Second Step (\( \mathbf{P}_2 \) to \( \mathbf{P}_3 \)):
      \[
      \mathbf{P}_3 = \mathbf{P}_2 + h \hat{\mathbf{r}}_2 = (0.32, 0.26) + 0.3 \cdot (-0.6, -0.8) = (0.32 - 0.18, 0.26 - 0.24) = (0.14, 0.02)
      \]
      \[
      z_3 = 4 - (0.14)^2 - (0.02)^2 = 4 - 0.0196 - 0.0004 = 3.98
      \]
      Thus, \( \mathbf{P}_3 = (0.14, 0.02, 3.98) \).
...
\begin{tikzpicture}
  \begin{axis}[
    ...
    \node[below right, red] at (axis cs:0.32,0.26,3.83) {\( \mathbf{P}_2 \)};
    \node[below left, red] at (axis cs:0.14,0.02,3.98) {\( \mathbf{P}_3 \)};

    % Label for the gradient vectors
    \node[above right, blue] at (axis cs:0.41,0.38,3.67) {$\mathbf{r}_1$};
    \node[above right, blue] at (axis cs:0.23,0.14,3.90) {$\mathbf{r}_2$};

  \end{axis}
\end{tikzpicture}
\caption{\textit{The gradient ascent of the two vectors found in this problem starting at point \( \mathbf{P}_1 = (0.5, 0.5) \).}}
\end{figure}
...
\]
defines a \textit{tangent hyperplane} to the graph of \( f \) at \(\mathbf{a}\). For \( n=1 \), this hyperplane is just a line (the tangent line). For \( n=2 \), it’s a plane (the tangent plane). For \( n>2 \), the object is an \((n\!-\!1)\)-dimensional hyperplane—a direct generalization of the idea of “flat” linear approximations to higher dimensions.
...
\begin{tikzpicture}
  ...
  \end{axis}
\end{tikzpicture}
\caption{\textit{The surface plot of $f(x,y) = \sin(x)\sin(y)e^{x^2 + y^2}$ with annotated extrema. No laughing, please.}}
...
\noindent The plot above illustrates the function \( f(x,y) = 2x^2 + 3y^2 - 4x - 6y + 10 \) over the region \( R \). The global minimum at \( (1, 1, 5) \) is marked with a \textbf{red circle}, and the global maximum at \( (3, 3, 25) \) with a \textbf{blue triangle}. Refer to the legend below the graph for clarification.
        ...
        \noindent The plot above illustrates the function \( f(x,y) = 2x^2 + 3y^2 - 4x - 6y + 10 \) over the region \( R \). The global minimum at \( (1, 1, 5) \) is marked in red, and the global maximum at \( (3, 3, 25) \) is marked in blue.
    \end{solution}
\end{example}
\exampleline
...
\]
Therefore we have $D = 2(-2) - (0^2) = -4 < 0$. Thus, confirming a saddle point. Our previous example we saw in Figure~\ref{fig:extrema_ex} we indeed have one saddle point. Let's find it!


\exampleline
\begin{example} Determine the saddle point of the function \( f(x,y) = \sin(x)\sin(y)e^{x^2 + y^2} \).
...
\begin{itemize}
    ...
    \item If \( D_3 < 0 \), the Hessian is \textbf{indefinite}, indicating a \textbf{saddle point}.
    \item If \( D_3 = 0 \), the Hessian is \textbf{semidefinite}, and the test is \textbf{inconclusive}.
\end{itemize}
...
\begin{enumerate}
    ...
    \begin{itemize}
        ...
        \[
        f(2, 3) = (2)^2 + (3)^2 - 4(2) - 6(3) + 13 = 4 + 9 - 8 - 18 + 13 = 1.
        \]
        \item The minimum cost is \(1\), achieved when \(x = 2\) and \(y = 3\).
    \end{itemize}
\end{enumerate}
\end{solution}
\end{example}
...
\noindent To derive this condition, consider the constrained optimization problem as finding the extrema of \( f(x, y) \) on the curve \( g(x, y) = c \). We construct the \textit{Lagrange function}: \index{Lagrange Function}
\[
...
\noindent Let's now try a problem that's a little messier and doesnt have a nice answer. We will need to use a computer to solve a polynomial using numerical approximation at the end. This is more representative a problem you may encounter in an actual professional setting.
...
\noindent For two constraints, we define the \textit{Lagrange function} as:
\[
...
\noindent Up until now, we have considered optimization problems with equality constraints, where conditions like \( g(x,y) = c \) must be met. However, many real-world problems also include \textit{inequality constraints}, such as \( h(x,y) \geq d \). These constraints arise naturally in scenarios where certain values should not fall below (or rise above) a specified level—for example, ensuring that a production level does not drop below a minimum quota, or that a budget does not exceed a certain maximum amount. \\

\noindent To handle such conditions, mathematicians and engineers use a more general framework known as the \textbf{Karush-Kuhn-Tucker (KKT)} conditions \index{Karush-Kuhn-Tucker Conditions}. The KKT conditions extend the idea of Lagrange multipliers to cover both equality and inequality constraints. In essence, they incorporate additional variables (called KKT multipliers) that must satisfy certain non-negativity requirements and complementary conditions. The key addition is that when a constraint is not “active” (i.e., it does not exactly hold at its boundary), its corresponding multiplier must be zero, ensuring that non-binding constraints do not influence the solution. \\

\noindent Although we will not dive deeply into the full derivation, here are the general steps and guidelines for dealing with inequality constraints:
\begin{itemize}
    \item \textbf{Formulate the Problem:} Start with your objective function \( f(x,y) \) and identify any inequality constraints, such as \( h(x,y) \geq d \).
    \item \textbf{Introduce KKT Multipliers:} Assign a KKT multiplier (often denoted \(\mu\)) to each inequality constraint. These multipliers must be non-negative.
    \item \textbf{Set Up the Lagrangian-like Function:} Similar to the case of equality constraints, form a function incorporating both the original objective and the constraints, but now include terms \(\mu \cdot [d - h(x,y)]\) that represent the inequality constraints. 
    \item \textbf{Derive the First-Order Conditions:} Take partial derivatives of this augmented function with respect to your variables and set them to zero. You also include conditions that relate the sign of \(\mu\) to whether the constraint is active.
    \item \textbf{Check Complementary Slackness:} The KKT framework requires that \(\mu [d - h(x,y)] = 0\). This means:
    \[
    \mu > 0 \implies h(x,y) = d \quad \text{(active constraint)},
    \]
    \[
    \mu = 0 \implies h(x,y) > d \quad \text{(inactive constraint)}.
    \]
    \item \textbf{Identify the Feasible and Optimal Points:} Solve the resulting system of equations and inequalities to find candidate solutions, then verify which candidate provides the best (minimum or maximum) objective value while respecting the constraints.
\end{itemize}

\noindent While we will not fully derive or prove the KKT conditions here, it is enough to recognize that they represent a natural extension of the methods we have discussed. When confronted with more intricate constraints—like ensuring \( h(x,y) \geq 0 \)—the KKT conditions guide us to solutions that honor these inequalities. Thus, just as Lagrange multipliers handle equality constraints elegantly, the KKT conditions handle both equality and inequality constraints, giving us a powerful toolbox to tackle a wide range of optimization problems.
...
\exampleline
\begin{example}
Consider a simplified Support Vector Machine (SVM) \index{Support Vector Machines} scenario where we aim to find the optimal parameter \( w \) that separates two data points with labels \( y_1 = 1 \) and \( y_2 = -1 \). Suppose the data points are \( x_1 = 2 \) and \( x_2 = -1 \). The objective is to maximize the margin between the two points, which can be formulated as minimizing the function:
\[
f(w) = \frac{1}{2}w^2,
\]
subject to the following inequality constraints ensuring that each point is correctly classified with a margin of at least 1:
\[
y_1(w \cdot x_1) \geq 1 \quad \text{and} \quad y_2(w \cdot x_2) \geq 1.
\]
Substituting the given values:
\[
1 \cdot (w \cdot 2) \geq 1 \quad \Rightarrow \quad 2w \geq 1,
\]
\[
-1 \cdot (w \cdot (-1)) \geq 1 \quad \Rightarrow \quad w \geq 1.
\]

\begin{solution}
\noindent We aim to minimize \( f(w) = \frac{1}{2}w^2 \) subject to the constraints:
\[
2w \geq 1 \quad \text{and} \quad w \geq 1.
\]
These can be rewritten as:
\[
h_1(w) = 2w - 1 \geq 0, \quad h_2(w) = w - 1 \geq 0.
\]
 
\begin{enumerate}
    \item \textbf{Formulate the Lagrangian:} Introduce KKT multipliers \( \mu_1 \) and \( \mu_2 \) for the two inequality constraints:
    \[
    \mathcal{L}(w, \mu_1, \mu_2) = \frac{1}{2}w^2 - \mu_1 (2w - 1) - \mu_2 (w - 1).
    \]
    
    \item \textbf{Derive the First-Order Conditions:} Take the derivative of \( \mathcal{L} \) with respect to \( w \) and set it to zero:
    \[
    \frac{\partial \mathcal{L}}{\partial w} = w - 2\mu_1 - \mu_2 = 0 \quad \Rightarrow \quad w = 2\mu_1 + \mu_2.
    \]
    
    \item \textbf{Complementary Slackness Conditions:} 
    \[
    \mu_1 (2w - 1) = 0, \quad \mu_2 (w - 1) = 0.
    \]
    \[
    \mu_1 \geq 0, \quad \mu_2 \geq 0.
    \]
    
    \item \textbf{Analyze Possible Cases:}
    
    \begin{itemize}
        \item \textbf{Case 1: Both Constraints are Active}
        
        If both constraints are active, then:
        \[
        2w - 1 = 0 \quad \Rightarrow \quad w = \frac{1}{2},
        \]
        \[
        w - 1 = 0 \quad \Rightarrow \quad w = 1.
        \]
        This leads to a contradiction since \( w \) cannot be both \( \frac{1}{2} \) and \( 1 \). Therefore, this case is not possible.
        
        \item \textbf{Case 2: Only \( h_1(w) \) is Active}
        
        If \( h_1(w) \) is active, then \( 2w - 1 = 0 \) and \( w > 1 \) (since \( h_2(w) \) is inactive).
        \[
        w = \frac{1}{2}.
        \]
        However, \( \frac{1}{2} < 1 \), which violates \( h_2(w) \geq 0 \). Therefore, this case is also not possible.
        
        \item \textbf{Case 3: Only \( h_2(w) \) is Active}
        
        If \( h_2(w) \) is active, then \( w - 1 = 0 \) and \( 2w - 1 > 0 \):
        \[
        w = 1.
        \]
        Check \( h_1(w) \):
        \[
        2(1) - 1 = 1 \geq 0 \quad \text{(satisfied)}.
        \]
        Substitute \( w = 1 \) into the first-order condition:
        \[
        1 = 2\mu_1 + \mu_2.
        \]
        Since \( h_1(w) \) is inactive (\( \mu_1 = 0 \)):
        \[
        1 = 0 + \mu_2 \quad \Rightarrow \quad \mu_2 = 1.
        \]
        \[
        \mu_1 = 0, \quad \mu_2 = 1.
        \]
        
        \item \textbf{Case 4: Both Constraints are Inactive}
        
        If both constraints are inactive, then \( \mu_1 = 0 \) and \( \mu_2 = 0 \). The first-order condition becomes:
        \[
        w = 0.
        \]
        However, check the constraints:
        \[
        2(0) - 1 = -1 < 0 \quad \text{and} \quad 0 - 1 = -1 < 0,
        \]
        which violate both constraints. Therefore, this case is not feasible.
    \end{itemize}
    
    \item \textbf{Determine the Optimal Solution:}
    
    The only feasible case is \textbf{Case 3}, where:
    \[
    w = 1, \quad \mu_1 = 0, \quad \mu_2 = 1.
    \]
    
    \item \textbf{Verify the Solution:}
    
    Substitute \( w = 1 \) back into the objective function:
    \[
    f(1) = \frac{1}{2}(1)^2 = \frac{1}{2}.
    \]
    This is the minimum value of \( f(w) \) subject to the given constraints.
\end{enumerate}

\noindent \textbf{Conclusion:} The optimal parameter is \( w = 1 \), which satisfies the inequality constraints \( 2w \geq 1 \) and \( w \geq 1 \). This solution minimizes the objective function while ensuring that both constraints are respected. In this simplified SVM scenario, \( w = 1 \) represents the parameter that maximizes the margin between the two data points while correctly classifying them.

\end{solution}
\end{example}
\exampleline

\noindent This simplified scenario captures the core idea: if unconstrained optimization leads us outside the allowed set, inequality constraints force us to choose the best feasible point. More sophisticated methods, such as the KKT conditions, allow us to systematically solve problems with multiple and more complex inequalities, a crucial step in handling real-world optimization tasks in fields like machine learning.
...
\paragraph{Range:}
\[
\range f = \{z \in \mathbb{R} \mid (x, y) \in \dom f \}.
\]
...
%lecture: Functions of Several Variables
    ...
    \[
    M = f(P,r,t) = P\frac{r(1+r)^{12t}}{12((1+r)^{12t}-1)}
    \]
...
%lecture: Limits and Continuity
    ...
    \item Prove that $f(x,y) = \frac{x^3y}{x^4 + y^2}$ is continuous at $(0,0)$.
...
\item Find $f_x$ and $f_y$ for $f(x,y) = x^2y + 3xy^2$.
    ...
    \item For the function $f(x,y) = x^y$, show that:
    \[
    y\frac{\partial f}{\partial x} = x\frac{\partial f}{\partial y}
    \]
    This is known as Euler's homogeneous differential equation.
    ...
    \item For the implicitly defined function $y(x)$ given by $x^2 + y^2 = \sin(xy)$, show that:
    \[
    \frac{d^2y}{dx^2}\bigg|_{(0,0)} = -1
    \]
    
    \item If $z$ is implicitly defined by $e^{xyz} = xyz + 1$, prove that:
    \[
    \frac{\partial z}{\partial x} \cdot \frac{\partial z}{\partial y} \cdot \frac{\partial z}{\partial z} = -1
    \]
...
%lecture: Extrema and Critical Points
    ...
    \item Show that $f(x,y) = (y-x^2)(y-2x^2)$ has exactly three critical points and classify each.
...
%lecture: Optimization
    ...
    \item Maximize the function \( f(x,y) = 3x + 4y \) subject to the inequality constraints:
    \[
    \begin{cases}
    x + 2y \geq 8 \\
    x - y \geq 2
