...
The study of vector-valued functions marks an important progression in our exploration of multivariable calculus, seamlessly combining geometry and calculus to describe motion, curves, and change in three-dimensional space. Interestingly, we have already encountered a special case of vector-valued functions in Chapter 1 when we discussed parametric equations of lines in space. At the time, we focused on their geometric interpretation without explicitly framing them as vector-valued functions. In essence, those parametric representations—where we expressed lines as functions of a parameter—were early examples of linear vector-valued functions. \\ \\ By revisiting and expanding on this foundation, we will now view these functions through a broader lens, exploring their behavior, calculus, and applications in motion and geometry. This chapter builds on that implicit understanding, introducing parametric equations in their full generality and extending calculus to these functions. Concepts like velocity, acceleration, and arc length emerge naturally from the derivatives and integrals of vector-valued functions, allowing us to model and analyze dynamic systems in physics and engineering with mathematical rigor. \\ \\ The connections we uncover here not only highlight the unifying power of mathematics but also provide a deeper appreciation for how foundational concepts like lines in space can evolve into tools for analyzing complex systems. By recognizing these links, students can better appreciate how the ideas in earlier chapters inform and enrich their understanding of new topics.
\newpage 
...
\begin{itemize}
    ...
    A parametric curve can often be reparameterized to simplify its representation or emphasize certain properties. For example, reparameterizing a curve by its arc length \( s \) provides a direct measure of distance traveled along the curve. Arc length reparameterization is a key topic in the calculus of vector-valued functions, as it allows for uniform treatment of curves regardless of their initial parameterization. We will explore this later.
\end{itemize}
...
\begin{itemize}
    ...
    \[
    \mathbf{r}(t) = \langle v_0 \cos(\theta) t, v_0 \sin(\theta) t - \frac{1}{2} g t^2, 0 \rangle,
    \]
...
\]
where \( f(t), g(t), \) and \( h(t) \) are differentiable scalar functions. The derivative of \( \mathbf{r}(t) \) with respect to \( t \) is defined component-wise as:
\[
\mathbf{r}'(t) = \langle f'(t), g'(t), h'(t) \rangle.
\]
This derivative represents the rate of change of the vector-valued function with respect to the parameter \( t \). Geometrically, \( \mathbf{r}'(t) \) is a vector tangent to the curve defined by \( \mathbf{r}(t) \) at the point corresponding to \( t \). Its direction indicates how the curve is changing at that point. For example, if \( \mathbf{r}'(t) \) points upward, the curve is ascending; if it points downward, the curve is descending. \\ \\ Higher derivatives are also defined component-wise. The second derivative of \( \mathbf{r}(t) \) is given by:
\[
\mathbf{r}''(t) = \langle f''(t), g''(t), h''(t) \rangle,
\]
which provides information about the concavity of the curve or how sharply it is bending at each point. These higher-order derivatives are critical for analyzing properties such as increasing or decreasing behavior, inflection points, and curvature. \\ \\ The concept of component-wise differentiation not only simplifies calculations but also lays the foundation for generalizing change in multidimensional contexts. By treating each coordinate function \( f(t), g(t), h(t) \) independently, we maintain a structured approach to understanding how the entire system evolves.
...
\]
where \( \mathbf{C} = \langle C_1, C_2, C_3 \rangle \) is a constant vector of integration. \\ \\ Integrals of vector-valued functions allow us to reconstruct curves, compute displacements, and analyze the accumulation of quantities across space. This process is inherently component-wise, treating each spatial dimension independently, while combining them to form the overall trajectory or system. \\ \\ Consider a general situation where a vector-valued function \( \mathbf{v}(t) \) represents some change, such as motion or growth, in three-dimensional space. Integrating \( \mathbf{v}(t) \) over time allows us to recover the function \( \mathbf{r}(t) \) that describes the system:
\[
\mathbf{r}(t) = \int \mathbf{v}(t) \, dt = \langle \int v_x(t) \, dt, \int v_y(t) \, dt, \int v_z(t) \, dt \rangle + \mathbf{r}_0,
\]
where \( \mathbf{r}_0 = \langle x_0, y_0, z_0 \rangle \) is the initial condition or position at \( t = 0 \). \\ \\ This approach highlights the power of vector-valued integration in understanding phenomena that depend on independent contributions along multiple dimensions. Similarly, if \( \mathbf{a}(t) = \mathbf{r}''(t) \) is a second derivative, integrating \( \mathbf{a}(t) \) twice recovers \( \mathbf{r}(t) \), with each step of integration requiring the addition of constants reflecting initial conditions. \\ \\ By framing integrals component-wise, we emphasize the generalizability of these operations to multidimensional systems, while preserving the simplicity and independence of single-variable integration.
...
\noindent The calculus of vector-valued functions has a wide range of applications in physics, engineering, and computer science. Some examples include:

\begin{itemize}
    \item \textbf{Kinematics:} Derivatives and integrals of position vectors describe velocity, acceleration, and displacement, essential for analyzing the motion of objects.
    \item \textbf{Electromagnetism:} In physics, vector-valued functions are used to model electric and magnetic fields, with integrals representing quantities like flux.
    \item \textbf{Computer Graphics:} Derivatives of vector-valued functions are used to compute normals and tangents, while integrals are employed in generating smooth curves and surfaces.
\end{itemize}

\noindent By understanding the calculus of vector-valued functions, we gain the ability to analyze and predict the behavior of dynamic systems in space, paving the way for more advanced topics in multivariable calculus.
...
\]
Geometrically, the velocity vector connects the rate of change of the coordinates \( x, y, \) and \( z \), capturing the instantaneous direction and rate at which the object moves along the curve. \\ \\ \textbf{The Acceleration Vector} \( \mathbf{a}(t) \) is defined as the derivative of the velocity vector:
\[
...
\begin{itemize}
    \item If the velocity vector \( \mathbf{v}(t) \) is constant, the object moves along the curve with uniform speed.
...
\[
\Delta U = m g z_{\text{max}} = m g \left( \frac{v_0^2 \sin^2 \theta}{2g} + z_0 \right) = \tfrac{1}{2} m v_0^2 \sin^2 \theta + m g z_0.
\]
...
\begin{enumerate}
    ...
    &= \dfrac{200}{9.8} - \dfrac{200}{9.8} \\
    &= \dfrac{200}{9.8} - \dfrac{200}{9.8} = 10.204 \, \text{m}.
    \end{aligned}
    \]
    ...
    \[
    t \left( 10\sqrt{2} - 4.9 t \right) = 0 \implies t_{\text{total}} = \dfrac{10\sqrt{2}}{4.9} \approx 2.04 \, \text{s}.
    \]
    ...
    \[
    x_{\text{range}} = v_{x0} \times t_{\text{total}} = 10\sqrt{2} \times 2.04 \approx 28.9 \, \text{m}.
    \]
...
\noindent As we extend our study of motion in space, we now introduce the TNB frame—a powerful tool for analyzing the geometry of curves and the motion of objects along those curves. The TNB frame consists of three mutually orthogonal vectors: the tangent vector, the normal vector, and the binormal vector. Together, these vectors form a moving frame that evolves along the curve, providing a dynamic coordinate system to describe its behavior. This section also introduces the Frenet-Serret equations, which govern the interplay between these vectors, and discusses applications of the TNB frame to motion in space.

\subsection*{The TNB Frame}

\noindent In previous sections, we discussed the derivatives of vector-valued functions, which naturally give rise to the tangent vector (\( \mathbf{T}(t) \)), and explored the normal vector (\( \mathbf{N}(t) \)) as a tool for understanding how curves bend. Building upon this foundation, we now introduce the binormal vector (\( \mathbf{B}(t) \)) to complete the TNB frame. Together, these vectors form an orthonormal basis that evolves along the curve, providing a dynamic coordinate system tied directly to the curve’s intrinsic geometry and motion. \\
...
\tikzset
{
  ...
  pics/vectors/.style={code=% #1 = angle
    {\draw[green!50!black,-latex,opacity=0.3] ({1.5*cos(#1)},{1.5*sin(#1)},1.5*#1/180) -- 
      ({0.6*cos(#1)},{0.6*sin(#1)},1.5*#1/180) node[pos=1.5] {$\textbf{N}$};
     \begin{scope}[shift={({1.5*cos(#1)},{1.5*sin(#1)},1.5*#1/180)},rotate around z=90+#1,canvas is xz plane at y=0]
       \draw[blue!50!black,-latex]    (0,0) --++ ({atan(1/pi)}:1.125) node[pos=1.4] {$\textbf{T}$};
       \draw[magenta!50!black,-latex] (0,0) --++ ({90+atan(1/pi)}:1.5) node[pos=1.3] {$\textbf{B}$};
     \end{scope}}}
}
...
\begin{enumerate}
    ...
    \[
    \mathbf{B}(1) = \left\langle \frac{3}{\sqrt{266}}, -\frac{3}{\sqrt{266}}, \frac{1}{\sqrt{266}} \right\rangle.
    \]
\end{enumerate}
...
\noindent Beyond position, velocity, and acceleration, the curvature of a curve provides insights into how sharply the trajectory bends at each point. It quantifies the rate at which the direction of the curve changes as you move along it. For a smooth curve defined by the vector-valued function \( \mathbf{r}(t) \), the curvature \( \kappa(t) \) at a point is given by:
\[
\kappa(t) = \frac{|\mathbf{r}'(t) \times \mathbf{r}''(t)|}{|\mathbf{r}'(t)|^3} = \frac{|\mathbf{v}(t) \times \mathbf{a}(t)|}{|\mathbf{v}(t)|^3} = \frac{|\mathbf{T}'(s)|}{|\mathbf{r}'(s)|}
\]
Here:
\begin{itemize}
    ...
    \item The cross product \( \mathbf{r}'(t) \times \mathbf{r}''(t) \) captures the tendency of the curve to twist out of a straight path, emphasizing the interaction between velocity and acceleration.
\end{itemize}
...
\begin{itemize}
    ...
    \item The curvature \(\kappa(x)\), shown as a red dashed line, which has peaks at \(x \approx 0\), \(x \approx 1.5\), and \(x \approx 2.5\), indicating regions of high curvature.
...
\noindent Similarly, as we have done with curvature, if we consider a curve in the form \( \mathbf{r}(t) = \langle t, f(t) \rangle \), we can derive an expression for torsion for any function \( f(t) \). For such a curve:
\[
...
% \pgfplotsset{trig format=rad} % Remove or comment out this line
...
\noindent The circular helix in Figure \ref{fig:curvature_torsion} illustrates a constant rate of twist, embodying the concept of torsion in a tangible form.
...
\noindent where \( x_i^* \) is some point in the \( i \)-th subinterval.  Note, that we replace the finite difference \( (\Delta y_i)^2 \) with the derivative \( f'(x_i^*) \) multiplied by the finite difference \( \Delta x_i \), following the limit definition of the derivative. This substitution simplifies the expression and emphasizes the relationship between the derivative and the arc length formula. 
...
\begin{enumerate}
    ...
    \]
    The negative sign arises because \( dx = -r \sin t \, dt \), and we reversed the limits when changing variables.
...
\[
\sqrt{ [x'(t)]^2 + [y'(t)]^2 } = \sqrt{ (2t)^2 + (3t^2)^2 } = \sqrt{ 4t^2 + 9t^4 } = t \sqrt{ 4 + 9t^2 }.
\]
...
\paragraph{Curvature:}
\[
\kappa(t) = \frac{|\mathbf{r}'(t) \times \mathbf{r}''(t)|}{|\mathbf{r}'(t)|^3} = \frac{|\mathbf{T}'(s)|}{|\mathbf{r}'(s)|}
\]
\[
\kappa(x) = \frac{|f''(x)|}{\left( 1 + [f'(x)]^2 \right)^{3/2}}.
\]
...
\paragraph{Fundamental Theorem of Calculus for Vector-Valued Functions:}
\[
...
% Vector-Valued Functions: Parametric Equations
    \item Determine whether the curve \( \mathbf{r}(t) = \langle \cos t, \sin t, t \rangle \) is smooth for all \( t \).
    ...
    \item Show that the curve \( \mathbf{r}(t) = \langle t, t^2, t^3 \rangle \) lies on the surface \( z = y^{3/2} \).
...
% Vector-Valued Functions: Calculus with Vector-Valued Functions
    ...
    \item Evaluate the integral \( \int_0^{\pi} \mathbf{r}(t) \times \mathbf{r}'(t) \, dt \) where \( \mathbf{r}(t) = \langle \cos t, \sin t, t \rangle \).
...
% Motion Along a Curve: Velocity, Acceleration, and Projectile Motion
    ...
    \item A particle moves along the curve \( \mathbf{r}(t) = \langle \cos t, \sin t, t \rangle \). Determine its speed and acceleration at \( t = \frac{\pi}{4} \).
...
% The TNB Frame and Curvature
    ...
    \item For the curve \( \mathbf{r}(t) = \langle \cos t, \sin t, t \rangle \), compute the tangent, normal, and binormal vectors at any \( t \).
    \item A particle moves along a helix \( \mathbf{r}(t) = \langle \cos t, \sin t, t \rangle \). Compute its curvature and torsion.
    ...
    \item Find the binormal vector \( \mathbf{B}(t) \) for the curve \( \mathbf{r}(t) = \langle \cos t, \sin t, t \rangle \) at \( t = \pi \).
...
% Arc Length and Reparameterization
    ...
    \item Reparameterize the curve \( \mathbf{r}(t) = \langle t, t^2, t^3 \rangle \) by arc length \( s \).
    \item Find the arc length parameterization of \( \mathbf{r}(t) = \langle \cos t, \sin t, t \rangle \).
...
% Arc Length and Reparameterization: Applications in Engineering Paths
    \item A robotic arm needs to follow a smooth path described by \( \mathbf{r}(t) = \langle t, t^2, t^3 \rangle \). Determine the arc length parameterization for \( t \in [0, 1] \).
    ...
    \item Optimize the path of a vehicle moving along the curve \( \mathbf{r}(t) = \langle t, t^2, t^4 \rangle \) to minimize the travel time given a constant speed.
    ...
    \item Reparameterize the curve \( \mathbf{r}(t) = \langle t, t^2, t^3 \rangle \) using the arc length from \( t = 0 \) to \( t = 1 \).
...
% Advanced Challenges
    ...
    \item Find the arc length parameterization of \( \mathbf{r}(t) = \langle t^3, t^2, t \rangle \) and use it to reparameterize the curve.
    \item Given \( \mathbf{r}(t) = \langle t, t^2, t^3 \rangle \), compute the curvature and the torsion of the curve at \( t = 1 \).
    \item A satellite orbits Earth following the curve \( \mathbf{r}(t) = \langle 7000\cos t, 7000\sin t, 0 \rangle \) km. Compute the arc length of one complete orbit.
    \item For the curve \( \mathbf{r}(t) = \langle t, \ln t, \sqrt{t} \rangle \), find the arc length from \( t = 0 \) to \( t = 2 \).
    ...
    \item A drone must follow a path described by \( \mathbf{r}(t) = \langle t, t^2, t^3 \rangle \) with constant speed. Find the appropriate reparameterization of the curve using arc length.
